...
public class BuserBean {
    ...
    /*
	 ...
	 그래서 Dto와 Entity의 차이로 인해 DTO와 Entity를 분리하는 것이 바람직
	 그래서 FromBean과 Entity의 차이로 인해 FormBean과 Entity를 분리하는 것이 바람직
     */
    
    // BuserBean(FormBean)을 Buser 엔티티로 변환하는 메소드
    public Buser toEntity() {
    	return Buser.builder()
    			...
    			.build(); // 죄종적으로 builder 객체에 설정된 값을 바탕으로 Buser 객체를 생성 후 반환
